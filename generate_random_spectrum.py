...
def generate_the_spectrum(wn=np.linspace(0, 1024, 1025),
    ...
    the_noise = noise_coef * np.ones(len(wn)) * np.random.normal(0, 1, len(wn))
    ...
    for pp in range(number_of_peaks):
        ...
        mp.peaks[pp].params.intensity = intensity
        ...
        mp.peaks[pp].peak_height = max_amplitude * np.random.sample()
...
#                 print('peak at {:.2f}, fwhm={:.2f}, height={:.2f}'.format(mp.peaks[pp].params.position, mp.peaks[pp].params.fwhm, mp.peaks[pp].peak_height))
    ...
    if return_order == 0:
        ...
        return mp, the_noise, baseline, bl_1stDer, bl_2ndDer
    else:
        return None
